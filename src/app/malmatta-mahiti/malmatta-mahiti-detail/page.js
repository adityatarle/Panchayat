...
// Updated initial states to include all fields from the screenshot
...
// Reusable component for consistent form fields with compact styling
...
export default function ModernMalmattaForm() {
    ...
    // --- FIX: Restore the state for the dependent dropdown ---
    ...
    const fetchNextSerial = async () => {
        try {
            ...
            if (!response.ok) throw new Error('Could not fetch serial number');
    ...
    // --- FIX: Restore the useEffect to update the dependent dropdown's options ---
    useEffect(() => {
        ...
        // Automatically select the first option when the parent dropdown changes
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!mainData.holderName.trim()) {
            alert('धारकाचे नाव आवश्यक आहे.');
            return;
        }
    ...
    const inputClass = "w-full p-1 border border-gray-400 rounded-sm shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-sm";
    const readOnlyClass = "bg-gray-200 cursor-not-allowed";
    const compactButtonClass = "px-4 py-1 text-sm font-semibold border border-gray-400 rounded-sm shadow-sm";

    return (
        <div className="bg-[#EAF3FE] min-h-screen p-2">
            <div className="bg-white container mx-auto p-4 border border-gray-400 shadow-lg">
                <h1 className="text-xl font-bold text-center mb-4">मालमत्ता माहिती</h1>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <fieldset className="p-2 border border-gray-300">
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-12 gap-x-4 gap-y-2">
                            ...
                            <div className="lg:col-span-9 space-y-2">
                                {/* All rows and fields in this section are correct */}
                                {/* ... rows 1 to 7 ... */}
                                ...
                                <div className="grid grid-cols-4 gap-x-4">
                                    <FormField id="street" label="रस्त्याचे नाव / गल्लीचा क्रमांक :" className="col-span-3"><select name="street" value={mainData.street} onChange={handleMainChange} className={inputClass}>{STREET_OPTIONS.map(opt => <option key={opt}>{opt}</option>)}</select></FormField>
                                </div>
                                <div className="grid grid-cols-2 gap-x-4">
                                    ...
                                    <FormField id="aadharNo" label="आधार क्रमांक :"><input type="text" name="aadharNo" value={mainData.aadharNo} onChange={handleMainChange} className={inputClass} maxLength="12" /></FormField>
                                </div>
                                <div className="grid grid-cols-2 gap-x-4">
                                    <FormField id="holderNameEnglish" label="२.मालमत्ता धारकाचे नाव (ENGLISH) :"><input type="text" name="holderNameEnglish" value={mainData.holderNameEnglish} onChange={handleMainChange} className={inputClass} /></FormField>
                                    <FormField id="mobileNo" label="मोबाईल नंबर :"><input type="tel" name="mobileNo" value={mainData.mobileNo} onChange={handleMainChange} className={inputClass} maxLength="10" /></FormField>
                                </div>
                            ...
                            <div className="lg:col-span-3 border-l-2 border-gray-300 pl-4 space-y-2">
                    ...
                    <fieldset className="p-2 border border-gray-300 relative" disabled={!isHolderInfoFilled}>
                        {!isHolderInfoFilled && (
                            <div className="absolute inset-0 bg-gray-100/80 flex items-center justify-center rounded-sm z-10 cursor-not-allowed">
                                <p className="text-gray-700 font-bold p-4 bg-yellow-200 border border-yellow-400 rounded">कृपया आधी मालमत्ता धारकाची माहिती भरा</p>
                            </div>
                        )}
                        ...
                        <div className="space-y-2">
                            ...
                            <fieldset className="p-2 border border-gray-300">
                                ...
                                <div className="flex items-end gap-x-4">
                                    ...
                                    <FormField id="length" label="लांबी"><input type="number" step="0.01" name="length" value={currentDescription.length} onChange={handleDescriptionChange} className={inputClass} style={{width: '100px'}}/></FormField>
                                    <FormField id="width" label="रुंदी"><input type="number" step="0.01" name="width" value={currentDescription.width} onChange={handleDescriptionChange} className={inputClass} style={{width: '100px'}}/></FormField>
                                    <FormField id="area" label="क्षेत्रफळ"><input type="text" name="area" value={currentDescription.area} className={`${inputClass} ${readOnlyClass}`} readOnly style={{width: '100px'}}/></FormField>
                            ...
                            <div className="grid grid-cols-4 gap-x-4">
                                <FormField id="constructionYear" label="बांधकाम वर्ष"><input type="number" name="constructionYear" value={currentDescription.constructionYear} onChange={handleDescriptionChange} className={inputClass} /></FormField>
                                ...
                                {/* --- FIX: Added onChange and dynamic options --- */}
                                <FormField id="dependentTaxType" label="७. रेडिरेकनर प्रमाणे भाग/उपविभाग">
                                    <select 
                                        name="dependentTaxType" 
                                        value={currentDescription.dependentTaxType} 
                                        onChange={handleDescriptionChange} 
                                        className={inputClass}
                                    >
                                        {dependentOptions.map(opt => <option key={opt} value={opt}>{opt}</option>)}
                                    </select>
                                </FormField>
                            </div>
                        </div>
                    ...
                    {/* Section 3: List of Added Descriptions */}
                    {descriptions.length > 0 && (
                        <div className="overflow-x-auto border border-gray-400">
                            <table className="min-w-full text-sm text-center">
                                <thead className="bg-gray-200">
                                    <tr>
                                        {['मि नं.', 'मि.प्रकार', 'मजला', 'लांबी', 'रुंदी', 'क्षेत्रफळ', 'वापर', 'बांधकाम वर्ष', 'कर सूट प्रकार', 'Action'].map(h =>
                                            <th key={h} className="p-1 border border-gray-300 font-semibold text-gray-700">{h}</th>)}
                                    </tr>
                                </thead>
                                <tbody>
                                    {descriptions.map((d, index) => (
                                        <tr key={d.id} className="bg-white">
                                            ...
                                            <td className="p-1 border border-gray-300 text-left">{d.propertyType}</td>
                                            <td className="p-1 border border-gray-300">{d.floor}</td>
                                            <td className="p-1 border border-gray-300">{d.length}</td>
                                            <td className="p-1 border border-gray-300">{d.width}</td>
                                            <td className="p-1 border border-gray-300">{d.area}</td>
                                            <td className="p-1 border border-gray-300">{d.usage}</td>
                                            <td className="p-1 border border-gray-300">{d.constructionYear}</td>
                                            <td className="p-1 border border-gray-300">{d.rebate}</td>
                                            <td className="p-1 border border-gray-300">
                                                <button type="button" onClick={() => handleEditDescription(d.id)} className="font-medium text-indigo-600 hover:underline">Edit</button>
                                            </td>
                                        </tr>
                                    ))}
                                </tbody>
                    ...
                    <div className="flex justify-between items-center">
                        ...
                        <button type="submit" disabled={isSubmitting || isSerialLoading} className="px-6 py-2 bg-green-600 text-white font-bold rounded-md shadow-sm hover:bg-green-700 disabled:bg-gray-400">
