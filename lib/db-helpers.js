...
export const dbOperations = {
  ...
  prepareMalmattaMahitiData: (formData) => {
    ...
    return {
      ...
      serialNo: sanitizeData.toString(formData.serialNo),
      period: sanitizeData.toString(formData.period),
      villageName: sanitizeData.toString(formData.villageName),
      propertyGroupEconomicCategory: sanitizeData.toString(formData.propertyGroupEconomicCategory),
      propertyNumber: sanitizeData.toString(formData.propertyNumber),
      wardNo: sanitizeData.toString(formData.wardNo),
      groupNo: sanitizeData.toString(formData.groupNo),
      propertyActivityNo: sanitizeData.toString(formData.propertyActivityNo),
      streetName: sanitizeData.toString(formData.streetName),
      laneNo: sanitizeData.toString(formData.laneNo),
      address: sanitizeData.toString(formData.address),
      propertyHolderName: sanitizeData.toString(formData.propertyHolderName),
      propertyHolderNameEnglish: sanitizeData.toString(formData.propertyHolderNameEnglish),
      ...
      waterSupply: sanitizeData.toString(formData.waterSupply),
      permanentAccountNumber: sanitizeData.toString(formData.permanentAccountNumber),
      tapNumber: sanitizeData.toString(formData.tapNumber),
      residentNumber: sanitizeData.toString(formData.residentNumber),
      toilet: sanitizeData.toString(formData.toilet),
      
      // Checkboxes (Right side options)
      planTax: formData.planTax === true || formData.planTax === 'true',
      diwaliTax: formData.diwaliTax === true || formData.diwaliTax === 'true',
      diwanAgam: formData.diwanAgam === true || formData.diwanAgam === 'true',
      defenseArea: formData.defenseArea === true || formData.defenseArea === 'true',
      midc: formData.midc === true || formData.midc === 'true',
      specialProperty: formData.specialProperty === true || formData.specialProperty === 'true',
  ...
  // Prepare Property Description data for database insertion
  preparePropertyDescriptionData: (propertyDescriptions, malmattaMahitiId) => {
    if (!propertyDescriptions || !Array.isArray(propertyDescriptions)) {
      return [];
    }

    return propertyDescriptions.map((desc, index) => ({
      malmattaMahitiId,
      serialNo: index + 1,
      propertyType: sanitizeData.toString(desc.propertyType),
      otherPropertyInfo: sanitizeData.toString(desc.otherPropertyInfo),
      floor: sanitizeData.toString(desc.floor),
      areaForPerson: sanitizeData.toFloat(desc.areaForPerson),
      tank: sanitizeData.toString(desc.tank),
      length: sanitizeData.toFloat(desc.length),
      width: sanitizeData.toFloat(desc.width),
      area: sanitizeData.toFloat(desc.area),
      usage: sanitizeData.toString(desc.usage),
      propertyAssessmentCategory: sanitizeData.toString(desc.propertyAssessmentCategory),
      assessmentYear: sanitizeData.toString(desc.assessmentYear),
      landAssessmentRatePerSqm: sanitizeData.toFloat(desc.landAssessmentRatePerSqm),
      landRate: sanitizeData.toFloat(desc.landRate),
      rateType: sanitizeData.toString(desc.rateType),
      shopRatePerSqft: sanitizeData.toFloat(desc.shopRatePerSqft),
      shopRatePerSqm: sanitizeData.toFloat(desc.shopRatePerSqm),
      taxAssessmentType: sanitizeData.toString(desc.taxAssessmentType),
      subdivision: sanitizeData.toString(desc.subdivision),
      rebateExemption: sanitizeData.toString(desc.rebateExemption)
    }));
  },
